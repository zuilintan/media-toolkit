...
def build_parser() -> argparse.ArgumentParser:
    # 延迟 import 子命令避免循环（mt.cli.name 反向 import 本模块的 validate_root）
    from mt.cli.cover   import cmd_cover,  add_cover_args
    from mt.cli.doctor  import cmd_doctor, add_doctor_args
    from mt.cli.meta    import cmd_meta,   add_meta_args
    from mt.cli.pack    import cmd_pack,   add_pack_args
    from mt.cli.name    import cmd_name,   add_name_args
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            '示例:\n'
            '  mt-cli name --root /path/to/manga --apply\n'
            '  mt-cli name --examples\n'
            '  mt-cli meta --root /path/to/cbz\n'
            '  mt-cli meta --root /path/to/cbz --apply\n'
            '  mt-cli meta --examples\n'
    ...
    p_name = sub.add_parser(
        ...
        epilog=(
            '常用模式:\n'
            '  mt-cli name --root <dir>         # 批量预览\n'
            '  mt-cli name --root <dir> --apply # 批量执行\n'
            '  mt-cli name --examples           # 内置解析示例\n'
        ),
    )
    ...
    p_meta = sub.add_parser(
        ...
        epilog=(
            ...
            '示例:\n'
            '  mt-cli meta --examples\n'
            '  mt-cli meta --root ./manga\n'
            '  mt-cli meta --root ./manga --apply\n'
        ),
    )
