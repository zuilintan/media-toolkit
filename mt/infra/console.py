...
提供:
  ...
  - render_progress()  — 单行刷新的进度条
  - iter_with_progress() — 批量执行 + 可选进度条 + 输出落日志的迭代器
  - SEP / SEP2         — 分隔线常量
...
def iter_with_progress(
    items:     Sequence[_T],
    do_one:    Callable[[_T], _R],
    *,
    log_lines: list[str] | None = None,
) -> Iterator[_R]:
    """逐项调用 ``do_one(item)`` 并 yield 其返回值。

    - ``log_lines is None`` → 直接执行，输出照常落到当前 sink，无进度条。
    - ``log_lines`` 给定列表 → 每项的 emit 输出被 capture 进 ``log_lines``，
      终端只刷新一行进度条。迭代结束后再 emit 一个换行收尾进度条。

    适用于大批量任务（comicinfo 批写、未来 rename 批量等）。
    """
    total = len(items)
