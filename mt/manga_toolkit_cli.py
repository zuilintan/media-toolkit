...
``mt-cli``（CLI 惯例）。两者解耦，互不影响。

子命令实现位于 mt.cli 包；本模块仅负责参数解析与调度。

用法示例:
  mt-cli sourcefile --drag                          # 循环拖入模式（推荐）
  mt-cli sourcefile --root /path/to/manga           # 批量预览
  mt-cli sourcefile --root /path/to/manga --apply   # 批量执行
  mt-cli sourcefile --rollback                      # 回退上次操作
  mt-cli sourcefile --list-sessions                 # 列出所有操作记录
  mt-cli sourcefile --examples                      # 内置解析示例

  mt-cli metadata --root /path/to/cbz               # 预览
  mt-cli metadata --root /path/to/cbz --apply       # 写入 ComicInfo.xml
  mt-cli metadata --examples                        # 内置示例

  mt-cli cover --root /path/to/cbz                  # 预览
  mt-cli cover --root /path/to/cbz --apply          # 写入封面 webp
  mt-cli cover --root /path/to/cbz --apply --smart  # smartcrop 模式
  mt-cli cover --drag                               # 循环拖入模式
