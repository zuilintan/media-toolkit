...
| `name` | 批量重命名源文件（.zip / .cbz），统一格式 |
| `cover` | 为 CBZ 写入 2:3 封面（源 `0001.*` → `0000.webp`；源 `cover.*` → `cover.webp`） |
| `meta` | 向 CBZ 写入 ComicInfo.xml 元数据 |
...
```bash
# 批量预览（不修改文件）
manga-cli pack --root /path/to/albums

# 批量执行（重命名 + 打包 + 删除源目录）
manga-cli pack --root /path/to/albums --apply

# 并行 plan（plan 阶段；≥ 4 个单位时才真正启用）
manga-cli pack --root /path/to/albums --apply --jobs 4
manga-cli pack --root /path/to/albums --apply --jobs 0   # 自动 min(cpu, 4)
```
...
```bash
# 批量预览（不修改文件）
manga-cli name --root /path/to/manga
...
manga-cli name --root /path/to/manga --jobs 4
```
...
```bash
# 预览
manga-cli cover --root /path/to/cbz
...
manga-cli cover --root /path/to/cbz --apply --jobs 4
manga-cli cover --root /path/to/cbz --apply --jobs 0   # 自动 min(cpu, 4)
```
...
manga-cli meta --root /path/to/cbz --apply --jobs 4
```
...
GUI 完全复用 `manga.workflow.{sourcefile,metadata,cover,pack}` 的 `plan_*` / `apply_*`
