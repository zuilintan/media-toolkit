...
```bash
# 批量预览（不修改文件）
mt-cli name --root /path/to/manga
...
mt-cli name --root /path/to/manga --jobs 4
```
...
mt-cli meta --root /path/to/cbz --apply --jobs 4
```
...
GUI 完全复用 `mt.workflow.{sourcefile,metadata,cover,pack}` 的 `plan_*` / `apply_*`
