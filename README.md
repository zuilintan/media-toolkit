...
| `rename-kit` | 批量重命名源文件（.zip / .cbz），统一格式 |
| `cover-kit` | 为 CBZ 写入 2:3 封面（源 `0001.*` → `0000.webp`；源 `cover.*` → `cover.webp`） |
| `meta-kit` | 向 CBZ 写入 ComicInfo.xml 元数据 |
...
| `Pillow` | `cover-kit` 子命令图像解码 / 裁剪 / WebP 编码 | CLI / GUI |
| `smartcrop` | `cover-kit --smart` 显著性裁剪 | CLI / GUI |
...
```bash
# 批量预览（不修改文件）
manga-cli pack-kit --root /path/to/albums

# 批量执行（重命名 + 打包 + 删除源目录）
manga-cli pack-kit --root /path/to/albums --apply

# 并行 plan（plan 阶段；≥ 4 个单位时才真正启用）
manga-cli pack-kit --root /path/to/albums --apply --jobs 4
manga-cli pack-kit --root /path/to/albums --apply --jobs 0   # 自动 min(cpu, 4)
```
...
```bash
# 批量预览（不修改文件）
manga-cli rename-kit --root /path/to/manga
...
manga-cli rename-kit --root /path/to/manga --jobs 4
```
...
```bash
# 预览
manga-cli cover-kit --root /path/to/cbz
...
manga-cli cover-kit --root /path/to/cbz --apply --jobs 4
manga-cli cover-kit --root /path/to/cbz --apply --jobs 0   # 自动 min(cpu, 4)
```
...
manga-cli meta-kit --root /path/to/cbz --apply --jobs 4
```
...
2. 调整选项（如 cover-kit 的 `smart` / `quality`，所有子命令的 `jobs`）
3. 点「扫描预览」→ 后台跑 plan，预览渲染到日志面板
4. 确认无误后点「执行」→ QMessageBox 二次确认 → 后台跑 apply → 可选移动

GUI 完全复用 `manga.workflow.{rename_kit,meta_kit,cover_kit,pack_kit}` 的 `plan_*` / `apply_*`
函数，通过 `base.console.set_output()` 接管文本输出到日志框，与 CLI 共用
所有渲染逻辑（`manga.presentation.view`）。
