...
>   入口：CLI `artifact-cli classify` / GUI `artifact-gui`；首次启动会自动在 `<user_config>/media-toolkit/config/artifact.json` 落盘空配置，编辑 `workdirs` 后即可使用（GUI 有「📂 打开 artifact.json」按钮）。
>
