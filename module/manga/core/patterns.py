"""正则表达式常量与规则表（集中存放，避免散落）。

规则表（:data:`CHAPTER_PATTERNS` / :data:`VOLUME_PATTERNS`）顺序敏感：
具体模式优先于模糊模式。文件按编号小节组织（``═`` 分隔线）。
"""

from __future__ import annotations
import re
from collections.abc import Callable
from module.manga.core.models import Chapter, Volume
...
#
#   :data:`COMPILATION_PARTS` — 合集类（总集篇），作为 [总集篇] tag 与 [zh] 同位
#   :data:`EXTRA_PARTS`       — 附录类（番外篇/后日谈），正篇外，在 CH. 位置输出
#   :data:`STRUCT_PARTS`      — 结构类（上/中/下篇），一卷内分段，在 CH. 位置输出
#
#   :data:`STANDALONE_PARTS`  — 三者并集，单独出现时提升为 ``MangaInfo.part_tag``
#
# 所有变体（おまけ/特典/番外編/前編/後編/総集篇/After 等）通过
# :func:`normalize_part` 统一收敛为上面六个标准词之一。
...
# ── 分编词规范化规则（顺序敏感） ───────────────────────────────────────────────
# 把任意变体收敛到 :data:`STANDALONE_PARTS` 中的标准词。
...
def normalize_part(s: str) -> str:
    """把分编词的各种变体收敛为 :data:`STANDALONE_PARTS` 中的标准词。"""
...
def is_compilation(part: str) -> bool:
    """规范化后的词是否属于 :data:`COMPILATION_PARTS`（总集篇）。"""
...
#
#   原始
#    │ :func:`wrap_bare_tags`               裸词 → [裸词]
#    │ :func:`promote_tags`                 [任意容器+关键字] → [标准tag]
#    │ :func:`normalize_chapter_tokens`     第N話/第N-M話 → CH.N / CH.N-M
#    │ :func:`normalize_translation_delim`  主¦译（缺右¦）→ 主 ¦译¦
#    ▼ :func:`normalize_subtitle_delimiters` -t-/―t―/—t— → ～t～（strip_tags 之后调用）
...
#   - 新增「容器+关键字 → 标准tag」：往 :data:`TAG_PROMOTE_RULES` 加一行
#   - 新增「最终形态检测」：往 :data:`LANG_TAGS` / :data:`UNCENSORED_RE` … 加一条
...
def wrap_bare_tags(s: str) -> str:
    """孤立（无容器）的已知标签关键字包裹为 ``[keyword]``，供 :func:`promote_tags` 处理。"""
...
TAG_PROMOTE_RULES: list[tuple[re.Pattern, str]] = [
    ...
    # ── zh ─────────────────────────────────────────────────────────────────────
    # 整段含「汉化/掃图/嵌字/重嵌/日语社/考生組/机翻/润色」的方括号 → [zh]
    # （吞掉社团名 / AI 标记等噪音前缀，如 [AI机翻润色]）
    (_pat(_bracket(
...
def _ch_bonus_range(m: re.Match) -> Chapter:
    """bonus + range（前置，少见）。分组: ``(bonus, start, end)``。"""
