...
规则:
  1. 标题 / 系列 / 译名中，空格以 ``・`` 替代；
  2. 合集类（``总集篇``）作为 ``[总集篇]`` tag 与 ``[zh]`` 同位（在语言标签前）；
  3. ``番外篇`` / ``后日谈`` / ``上篇`` / ``中篇`` / ``下篇`` 与 ``CH.`` 同级且互斥。
"""

from __future__ import annotations

from module.manga.core.models import MangaInfo
from module.manga.core import patterns as P
from module.manga.naming.text import dot, norm_punct


def build_new_name(info: MangaInfo) -> str:
    """根据 :class:`~module.manga.core.models.MangaInfo` 拼合目标名称（不含后缀）。
    ...
    - 无 ``publisher`` → ``[作者]``
    """
    head = (f'[{info.publisher} ({info.author})]'
            if info.publisher else f'[{info.author}]')
    parts: list[str] = [head, dot(info.main_title)]
